...
class Parte2Exercicio3Test extends TestCase
{
    use RefreshDatabase; 

    /** @test */
    public function CorrigeExercicio2Parte3()
    {
        $dados = [
        ...
        $response = $this->getJson('parte2/exercicio3/29-09-2022');

        $response->assertStatus(200);

        $response->assertJsonPath('0.nome', expect: 'Alexandre');
    $response->assertJsonPath('1.nome', 'Alice');
    $response->assertJsonPath('2.nome', 'Bruna');
    $response->assertJsonPath('3.nome', 'Bruno');
    $response->assertJsonPath('4.nome', 'João');
    }
}