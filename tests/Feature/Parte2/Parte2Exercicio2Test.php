...
class Parte2Exercicio2Test extends TestCase
{
    use RefreshDatabase; 

    /** @test */
    public function CorrigeExercicio2Parte2()
    {
        $dados = [
        ...
        $response = $this->getJson('parte2/exercicio2');

        $response->assertStatus(200);

        $response->assertSeeInOrder(['João', 'Mário', 'Bruna']); 
        
        $response->assertDontSee('Gabi');
